...
def fast_rcnn_losses(cls_score, bbox_pred, label_int32, bbox_targets,
    ...
    elif cfg.MODEL.LOSS_TYPE == 'focal_iou':
        focal_iou_loss_bbox = net_utils.compute_focal_iou(
            bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights,
            transform_weights=cfg.MODEL.BBOX_REG_WEIGHTS)
        loss_bbox = focal_iou_loss_bbox 
    elif cfg.MODEL.LOSS_TYPE == 'focal_diou':
        _, focal_diou_loss_bbox = net_utils.compute_focal_diou(
            bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights,
            transform_weights=cfg.MODEL.BBOX_REG_WEIGHTS)
        loss_bbox = focal_diou_loss_bbox
    elif cfg.MODEL.LOSS_TYPE == 'focal_ciou':
        _, focal_ciou_loss_bbox = net_utils.compute_focal_ciou(
            bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights,
            transform_weights=cfg.MODEL.BBOX_REG_WEIGHTS)
        loss_bbox = focal_ciou_loss_bbox
