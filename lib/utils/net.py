...
def compute_focal_diou(output, target, bbox_inside_weights, bbox_outside_weights,
                transform_weights=None):
    if transform_weights is None:
        transform_weights = (1., 1., 1., 1.)
    x1, y1, x2, y2 = bbox_transform(output, transform_weights)
    ...
    # diouk = ((1 - diouk) * iou_weights).sum(0) / output.size(0)
    confidence = (diouk + 1) / 2
    diouk = torch.pow((1 - 0.7 * confidence), 2) * (1 - diouk)
    diouk = (diouk * iou_weights).sum(0) / output.size(0)
...
#     # temporary the number is fixed
#     confidence = (diouk + 1) / 2
#     # loss = 2.47 * torch.pow((1 - 0.5 * confidence), 2) * (1 - diou)
#     # loss = torch.sum(loss)
#     focal_diouk = (torch.pow((1 - 0.5 * confidence), 2) * (1 - diouk) * iou_weights).sum(0) / output.size(0)
...
def compute_focal_ciou(output, target, bbox_inside_weights, bbox_outside_weights,
                transform_weights=None):
    if transform_weights is None:
        transform_weights = (1., 1., 1., 1.)
    ...
    # temporary fix the hyper-param of focal loss
    confidence = (2 + ciouk) / 3
    ciouk = (torch.pow((1 - 0.5 * confidence), 2) * (1 - ciouk) * iou_weights).sum(0) / output.size(0)

    return iouk, ciouk

def compute_focal_iou(output, target, bbox_inside_weights, bbox_outside_weights,
                transform_weights=None):
    if transform_weights is None:
        transform_weights = (1., 1., 1., 1.)
    ...
    iou_weights = bbox_inside_weights.view(-1, 4).mean(1) * bbox_outside_weights.view(-1, 4).mean(1)
    confidence = iouk
    focal_iouk = (torch.pow((1 - 0.5 * confidence), 2) * (1 - iouk) * iou_weights).sum(0) / output.size(0)
